...
def calculate_age(dob)
  if (dob)
    ...
    response = response.to_s.rjust(3, '0')
  end
end

def translate_tobacco(tobaccoUse) 
  case tobaccoUse
  when 'Yes'
    'Y'
  when 'No'
  ...
  begin 
    ...
    list&.each_with_index do |message, i|
      ...
      when 'Information'
        ...
        composedMessage = 'Information from web service: Message: ' << msg.to_s if msg
        Views.configuration.logger.error "Error occured while contacting VIEWS: " + composedMessage.to_s if composedMessage
      ...
      when 'Spelling'   
        ...
        composedMessage = 'Spelling Suggestions: For field: ' << field << ', Term: ' << term << ', Suggestions: ' 
        # probably a better way to do this, just getting the suggestions in an array
        # so I can use join() on it
        suggestionArr = []
        suggestions.each do |s|
          suggestionArr << s.text
        end
        composedMessage << suggestionArr.join(',')
        messages[i] = composedMessage
        Views.configuration.logger.info("composedMessage " + composedMessage&.to_s)
      when 'RareWord'
        suggestions = message.xpath('./WebMMDS:Suggestion', WebMMDS: 'WebMMDS')
        msg = message.at_xpath('./WebMMDS:Message[@level=3]', WebMMDS: 'WebMMDS')
        ...
        composedMessage = 'Rare Word: For field: ' << field << ', Term: ' << term << ', Message: ' << msg << ', Suggestions: ' 
        suggestionMessage = suggestionArr.join(',')
        composedMessage << suggestionMessage
        messages[i] = composedMessage
      when 'Surveillance'
        msg = message.at_xpath('./WebMMDS:Message[@level=3]', WebMMDS: 'WebMMDS')
        composedMessage = 'Surveillance Word: For field: ' << field << ', Term: ' << term << ', Message: ' << msg
        messages[i] = composedMessage
      else 
      end
    end
